(() => {
"use strict";
const COLORS=["red","yellow","green","blue","purple","cyan"], LEVEL_COUNT=100, SAVE_KEY="chromatic-shift-save-v1";
const state={screen:"home",currentLevel:1,board:[],selected:null,score:0,moves:0,target:0,cleared:0,combo:0,busy:false,boosterMode:null,pendingBooster:null,save:loadSave()};
...
function loadSave(){try{const s=JSON.parse(localStorage.getItem(SAVE_KEY)||"null");if(s&&s.version===1)return s}catch(e){}return{version:1,unlocked:1,stars:{},best:{},coins:50,boosters:{shuffle:2,hammer:1,colorbomb:1},tutorialSeen:false};}
...
function config(level){
 const colors=Math.min(6,4+Math.floor((level-1)/18));
 return{level,size:8,colors,moves:Math.max(18,31-Math.floor(level/18)),target:24+Math.floor(level*1.35),difficulty:level<=10?"Warm-up":level<=30?"Flow":level<=60?"Focus":level<=85?"Expert":"Master"};
}
...
function updateGame(){
 ...
 el.level.textContent=state.currentLevel;el.objective.textContent="Clear "+state.target+" tiles";el.objectiveLabel.textContent="Targets";el.objectiveValue.textContent=Math.min(state.cleared,state.target)+" / "+state.target;el.bar.style.width=(ratio*100)+"%";el.score.textContent=state.score.toLocaleString();el.moves.textContent=state.moves;el.coin.textContent=state.save.coins;el.shuffle.textContent=state.save.boosters.shuffle;el.hammer.textContent=state.save.boosters.hammer;el.bomb.textContent=state.save.boosters.colorbomb;
}
...
function showMap(){
 ...
 for(let i=1;i<=LEVEL_COUNT;i++){const unlocked=i<=state.save.unlocked,stars=state.save.stars[i]||0,b=document.createElement("button");b.className="level-node"+(stars?" completed":"")+(i===state.save.unlocked?" current":"")+(unlocked?"":" locked");b.disabled=!unlocked;b.innerHTML=unlocked?'<span class="num">'+i+'</span><span class="stars">'+("★".repeat(stars)+ "☆".repeat(3-stars))+'</span><span class="difficulty">'+config(i).difficulty+"</span>":'<span class="lock">🔒</span><span class="num">'+i+"</span>";b.addEventListener("click",()=>startLevel(i));el.levelMap.appendChild(b);}
